    ...
    def __init__(
        ...
        if act:
            ...
            print("  [TL] ElementwiseLayer %s: size:%s fn:%s" % (
                self.name, layer[0].outputs.get_shape(), combine_fn.__name__))  # ElementwiseLayer resnet_v1_50/block1/unit_1/bottleneck_v1/combine_layer: size:(?, 56, 56, 256) fn:add
        ...
        for l in layer[1:]:  # residual (?, 56, 56, 256)
            ...
            self.outputs = combine_fn(self.outputs, l.outputs, name=name)  # shortcut (?, 56, 56, 256) + residual (?, 56, 56, 256)
        if act:
        ...
        self.all_layers = list_remove_repeat(self.all_layers)  # 剔除掉重复的layers，resnet_v1_50/conv1，resnet_v1_50/bn0，resnet_v1_50/prelu0
...
class BatchNormLayer(Layer):
    ...
    def __init__(
            ...
            gamma_init=tf.random_normal_initializer(mean=1.0, stddev=0.002),    # bn层的参数 y = gamma * x' + beta，tf.ones_initializer,
            # dtype = tf.float32,
        ...
        with tf.variable_scope(name) as vs:  # 在命名空间'bn0'下，创建以下下变量
            axis = list(range(len(x_shape) - 1))  # cnn：[0, 1, 2]，fc是[0] https://www.cnblogs.com/ranjiewen/articles/7748232.html 这个里面维度是(n,c,h,w)所以分通道求均值方差是axis=(0，2，3），而本项目是(n,h,w,c)，因此均值方差是axis=(0,1,2)
            # fc axis = [0]
            ## 1. beta, gamma
            ...
            mean, variance = tf.nn.moments(self.inputs, axis)  # cnn (?,112,112,64), axis=[0,1,2] ; for i in 64 : mean(?,112,112), var(?,112,112)，out = (64:), (64:)， Tensor("resnet_v1_50/bn0/moments/Squeeze:0", shape=(64,), dtype=float32)，Tensor("resnet_v1_50/bn0/moments/Squeeze_1:0", shape=(64,), dtype=float32)
            try:  # TF12
                update_moving_mean = moving_averages.assign_moving_average(moving_mean, mean, decay,
                                                                           zero_debias=False)  # if zero_debias=True, has bias
                update_moving_variance = moving_averages.assign_moving_average(
                    moving_variance, variance, decay, zero_debias=False)  # if zero_debias=True, has bias，如果想提高稳定性，zero_debias设为true
                # print("TF12 moving")
            ...
            if trainable:
                ...
                self.outputs = act(
                    tf.nn.batch_normalization(self.inputs, moving_mean, moving_variance, beta, gamma, epsilon))  # Tensor("resnet_v1_50/bn0/Identity:0", shape=(?, 112, 112, 64), dtype=float32)
        ...
        self.all_layers = list(layer.all_layers)  # <class 'list'>: [<tf.Tensor 'resnet_v1_50/conv1/Identity:0' shape=(?, 112, 112, 64) dtype=float32>]
        self.all_params = list(layer.all_params)  # <class 'list'>: [<tf.Variable 'resnet_v1_50/conv1/W_conv2d:0' shape=(3, 3, 3, 64) dtype=float32_ref>]
        self.all_drop = dict(layer.all_drop)
        self.all_layers.extend([self.outputs])    # <class 'list'>: [<tf.Tensor 'resnet_v1_50/conv1/Identity:0' shape=(?, 112, 112, 64) dtype=float32>, <tf.Tensor 'resnet_v1_50/bn0/Identity:0' shape=(?, 112, 112, 64) dtype=float32>]
...
def conv2d_same(inputs, num_outputs, kernel_size, strides, rate=1, w_init=None, scope=None, trainable=None):
    ...
    if strides == 1:
        ...
        if rate == 1:
            ...
            nets = tl.layers.AtrousConv2dLayer(inputs, n_filter=num_outputs, filter_size=(kernel_size, kernel_size),
                                               b_init=None,
                                               rate=rate, act=None, W_init=w_init, padding='SAME', name=scope)  # (?, 114, 114, 64) >> (?, 56, 56, 256)
            nets = BatchNormLayer(nets, act=tf.identity, is_train=True, trainable=trainable,
...
def bottleneck_IR(inputs, depth, depth_bottleneck, stride, rate=1, w_init=None, scope=None, trainable=None):
    ...
    with tf.variable_scope(scope, 'bottleneck_v1') as sc:
        ...
        if depth == depth_in:
            ...
            shortcut = tl.layers.Conv2d(inputs, depth, filter_size=(1, 1), strides=(stride, stride), act=None, W_init=w_init, b_init=None, name='shortcut_conv', use_cudnn_on_gpu=True)  # (?, 112, 112, 64)>> (?, 56, 56, 256), 缩面积/2 升维*4, ksize=1感受野是1只为了变换形状以便后边对应元素相加
        ...
        residual = tl.layers.Conv2d(residual, depth_bottleneck, filter_size=(3, 3), strides=(1, 1), act=None, b_init=None, W_init=w_init, name='conv1', use_cudnn_on_gpu=True)  # (?, 112, 112, 64)>> (?, 112, 112, 64),ksize=3感受野大吃周边信息
        residual = BatchNormLayer(residual, act=tf.identity, is_train=True, trainable=trainable, name='conv1_bn2')  # (?, 112, 112, 64)>> (?, 112, 112, 64)
        # bottleneck prelu
        residual = tl.layers.PReluLayer(residual)  # (?, 112, 112, 64)>> (?, 112, 112, 64)
        # bottleneck layer 2
        residual = conv2d_same(residual, depth, kernel_size=3, strides=stride, rate=rate, w_init=w_init, scope='conv2', trainable=trainable)    # (?, 112, 112, 64) >> (?, 56, 56, 256) ，把吃过周边信息的residual通过cnn变换为shortcut形状，以便对应元素相加
        output = ElementwiseLayer(layer=[shortcut, residual], combine_fn=tf.add, name='combine_layer', act=None)  #   # shortcut (?, 56, 56, 256) + residual (?, 56, 56, 256)
        return output
...
def resnet(inputs, bottle_neck, blocks, w_init=None, trainable=None, reuse=False, keep_rate=None, scope=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):  # 在scope = resnet_v1_50，的命名空间中，创建一下变量：
        ...
        if bottle_neck:
            net = tl.layers.Conv2d(net_inputs, n_filter=64, filter_size=(3, 3), strides=(1, 1), act=None, W_init=w_init, b_init=None, name='conv1', use_cudnn_on_gpu=True)  # (?, 112, 112, 3) > (?, 112, 112, 64)
            net = BatchNormLayer(net, act=tf.identity, name='bn0', is_train=True, trainable=trainable)  # (?, 112, 112, 64) > (?, 112, 112, 64)
        ...
        for block in blocks:  # Block(scope='block1', unit_fn=<function bottleneck_IR at 0x14900d950>, args=[{'depth': 256, 'depth_bottleneck': 64, 'stride': 2, 'rate': 1}, {'depth': 256, 'depth_bottleneck': 64, 'stride': 1, 'rate': 1}, {'depth': 256, 'depth_bottleneck': 64, 'stride': 1, 'rate': 1}])
            with tf.variable_scope(block.scope):  # 在'block1' 命名空间下，创建以下下内容，建立block串
                for i, var in enumerate(block.args):  # 0； <class 'dict'>: {'depth': 256, 'depth_bottleneck': 64, 'stride': 2, 'rate': 1}
                    with tf.variable_scope('unit_%d' % (i + 1)):  # unit_1
                        net = block.unit_fn(net, depth=var['depth'], depth_bottleneck=var['depth_bottleneck'], w_init=w_init, stride=var['stride'], rate=var['rate'], scope=None, trainable=trainable)
        ...
        net = tl.layers.ReshapeLayer(net, shape=[-1, net_shape[1] * net_shape[2] * net_shape[3]], name='E_Reshapelayer')  # 7*7*2048 = (?, 100352)
...
def get_resnet(inputs, num_layers, type=None, w_init=None, trainable=None, sess=None, reuse=False, keep_rate=None):
    if type == 'ir':
        unit_func = bottleneck_IR  # ？？？为啥不往函数里面跳呢？
    elif type == 'se_ir':
    ...
    net = resnet(inputs=inputs,
                 bottle_neck=True,  # T/F分别表示：resnet两种bloack单元，https://blog.csdn.net/weixin_42486685/article/details/84789740
                 blocks=blocks,
                 w_init=w_init,
                 trainable=trainable,
                 reuse=reuse,
