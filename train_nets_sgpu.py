...
if __name__ == '__main__':
    ...
    images = tf.placeholder(name='img_inputs', shape=[None, *image_size, 3],
                            dtype=tf.float32)  # Tensor("img_inputs:0", shape=(?, 112, 112, 3), dtype=float32)
    labels = tf.placeholder(name='img_labels', shape=[None, ],
                            dtype=tf.int64)  # Tensor("img_labels:0", shape=(?,), dtype=int64)
    ...
    test_net = get_resnet(images, net_depth, type='ir', w_init=w_init_method, trainable=False, reuse=True,
            ...
            try:
                ...
                if count > 0 and count % show_info_interval == 0:
                    print(time.strftime("%Y%m%d%H%M%S", time.localtime()),
                          ' epoch %d, total_step %d, total loss is %.2f , inference loss is %.2f, weight deacy '
